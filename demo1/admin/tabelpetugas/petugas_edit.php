...
<body>
  ...
  <div class="container">
    ...
    <div class="kotak-atas">
        <div class="text">
          <h2 class="title-edit-data">Ubah Data Petugas</h2>
        </div>
        <form method="post">
        <div class="form-input">
            ...
            <div class="right-coloumn">
                <div class="form-group">
                    ...
                    <textarea class="input-edit" name="address" cols="30" rows="10" required><?= $data_tampil['alamat']?>
                    </textarea>
                    </div>
                <div class="form-group">
  ...
  if(isset($_POST['update'])){
    ...
    $sql_update="UPDATE data_user SET nik='$nik', nama='$nama', email='$email', telepon='$telepon',alamat='$address',rt='$rt' WHERE id_user='$id'";
    $query_update=mysqli_query($conn,$sql_update);
    if(!$query_update){
        echo "<script>alert('Maaf terjadi sedikit error')</script>";
    }else{
        echo "<script>alert('Ubah Data Berhasil')
        window.location.href='petugas_data.php'</script></script>";  
    }
}
  ?>
